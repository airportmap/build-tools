...
async function buildScss ( config, compressed = false ) {
    ...
    const cleanCSS = new CleanCSS( { compatibility: '*', level: 2 } );
    const res = [];
    ...
    for ( const file of files ) {
        ...
        try {
            ...
            const output = compressed ? cleanCSS.minify( result.css ) : result.css;

            const out = await saveFile( css, file, outDir, output.styles );
