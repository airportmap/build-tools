const { resolvePattern } = require( './config' );
...
async function compressJsonIfNeeded ( filePath, content ) {

    await writeFile(
        filePath,
        process.argv.includes( '--compressed' )
...
async function downloadFile ( filePath, targetPath ) {
    ...
    await compressJsonIfNeeded( targetPath, content );
